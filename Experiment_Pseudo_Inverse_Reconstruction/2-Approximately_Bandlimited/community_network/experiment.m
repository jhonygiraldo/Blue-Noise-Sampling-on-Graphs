...
m = [20:20:200];
%%
filter_approximately_bandlimited = ones(N,1);
for(i=bandwidth:N)
    filter_approximately_bandlimited(i) = exp(-4*(G.e(i)-G.e(bandwidth)));
end
%%
...
for(p=1:size(m,2))
    ...
    parfor(h=1:repetitions)
        ...
        %%
        x_hat = normrnd(1,0.5,[N,1]);
        x_hat = x_hat.*filter_approximately_bandlimited;
        x = G.U*x_hat;
        %%
