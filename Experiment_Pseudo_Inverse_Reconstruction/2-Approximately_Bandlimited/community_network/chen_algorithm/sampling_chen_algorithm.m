function sampling_operator = sampling_chen_algorithm(V_K,m,N)
...
while(length(sampled_indexes) < m)
    ...
    sampled_indexes = [sampled_indexes;possible_indexes(index_max)];
    possible_indexes = setdiff(possible_indexes,possible_indexes(index_max));
    %%
    M = zeros(1,N);
